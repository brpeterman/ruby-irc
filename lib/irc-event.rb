# Data type for IRC events. Stores the IRC command, a human-readable
# translation of the command, the command parameters, where the event
# originated, and when it was received.
# Also contains a link to the next event in the event queue.
module RubyIRC
  class IRCEvent
    attr_reader :from, :command, :rawcom, :params, :time
    attr_accessor :next

    # Create a new event. Stores the command in @rawcom and a
    # human-readable version in @command. Also records the time
    # of the event and sets up a link to the next event in the
    # queue.
    # [command] IRC command associated with this event.
    #           Typically a numeric command.
    # [params]  Array of parameters to the command.
    # [from]    If the event specifies an originator, specify it here.
    def initialize(command, params, from=nil)
      @from = from
      @rawcom = command
      ...
      if (command == 'PRIVMSG' or command == 'NOTICE') then
        if ((params[1])[0] == ''[0]) then
          to = params[0]
          params[1] = (params[1])[1..params[1].length-2]
          params = params[1].split(/ /, 2);
          @command = 'ctcp_'+params[0].downcase
        ...
        if (Events::Map[command]) then
          @command = Events::Map[command]
        else
          @command = command.downcase
        end
      end
      ...
      #$stderr.puts "    Params: "+@params.join(",")
      #$stderr.puts "    From: "+@from unless !from
    end
  ...
  # Holds a map of IRC command numbers to human-readable names.
  class Events
    # Copied shamelessly from Perl's Net::IRC::Event, because only one
    # person should /ever/ have to do something like this.
    Map = {
      '001' => "welcome",
      '002' => "yourhost",
      '003' => "created",
